...
### `dump`
Выгружает данные из таблиц в файлы с именами _[имя_базы]-[имя_таблицы].json_. Таблицы можно указать в ключе `--scope`.

### `check`
Проверяет наличие синхронизированных элементов. Если указан ключ `--fix`, корректирует флаг синхронизации в таблицах. 
...
### `--scope [scope]`
Позволяет указать тип элементов, с которыми надо работать: `items`, `albums`, `links` или `all` (по умолчанию).
...
### `--fromdays [N]`
Будут синхронизироваться элементы за последние `N` дней. Если также указан ключ `--start`, то он  игнорируется.
...
### `--fix`
Для некоторых команд указывает, что необходимо устранить найденные проблемы.
